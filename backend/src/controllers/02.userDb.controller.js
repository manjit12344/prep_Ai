...
export async function interviewResponse(req, res) {
    ...
    try {
        if (!flag) {   //first time starting interview!!
            ...
            aiChat = await first(preHistory, preDefined, type, level, company);

            const rightResponse = await handelFirst(nId, aiChat.nextQuestion);
            return res.json({
                interviewId: nId,
                id: rightResponse,
        ...
        aiChat = await runNow(history, sanitizedResponse, type, level, company);
